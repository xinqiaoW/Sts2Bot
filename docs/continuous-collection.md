...
正常持续采集使用 4 个 worker（runtime-worker-26 至 29），每个使用独立 Wine 环境；另一个独立的 25 worker 池只做历史种子补采。各 worker 按真实、真实、原变异、专项变异循环领取任务，空队列允许借用。两个变异队列各低于 600 个 pending 时按策略补充，不要求真实队列为空。详见[来源与调度](source-versions-and-scheduling.md)、[变异规则](mutations-8s.md)。
...
后续采集读取 `configs/real-runs-8s.json` 的 `encounter_seed_policy`：直接引用同目录 `mutations-targeted-8s.json` 的 43 个编组，每对安排 24 个战斗种子，其余编组保持 4 个。真实导入、普通变异和专项变异共用该规则；前 4 个种子及任务 ID 保持兼容。配置在进程启动时加载，部署后需按既有流程重启控制器才能用于新增任务。已经导入的来源不会因这项配置变化自动重扫。正常持续采集按构筑 ID 整体去重：真实导入检查真实、普通变异和专项变异三个活动库，重复构筑不再添加编组或种子任务。真实库中的重复构筑仍保留来源记录以便追溯；两个变异生成器沿用原跨库构筑去重。新构筑才按目标 24 / 其他 4 的规则排队。

历史数据补采是独立的一次性操作，见 [补采工具说明](../tools/one_off/README.md)。持续采集程序不导入或调用补采工具。
...
正常持续采集配置为 4 个独立 runtime（worker-26 至 29），历史补采另用 25 个 runtime，两者的 Wine prefix 和 data_dir 不重叠。正常采集继续原来的三个活动库、来源游标、2:1:1 领取比例和变异策略；先处理已有待采队列，再按水位导入或生成新构筑。保持 8 秒搜索、DOP 1、collect-only，不启动训练。状态与启动记录位于 `data/new-build-collection-20260919/`。
