# 抽象整局模拟器 v0（Silent A10）

不启动游戏，用 Python 模拟一整局（三幕）的**战斗外决策**，战斗结果交给可插拔的战斗预言机（默认是训练好的预期掉血模型 `F`，`train.predict`）。它是强化学习环境的底座：每局几毫秒（经验预言机）到几十毫秒（`F`，批量 GPU 推理）。

## 数据来源与设计原则

游戏目录（`catalogs/game-0.111.0.raw.json`）只给出卡牌/遗物/遭遇的**集合**，不含生成权重。所有战斗外规则因此都从公开 Spire Codex `.run` 历史里**经验拟合**（`sim.tables`），不猜规则：
