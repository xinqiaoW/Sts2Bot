...
|---|---|---|---|---|---|---|
| 绝对误差 | 0.97 | 2.49 | 5.93 | 11.56 | 16.18 | 26.43 |

|误差| ≤ 2 HP 的 pair 占 43.5%，≤ 5 HP 占 70.3%，> 15 HP 占 5.9%。

该快照多数构筑–目标使用 4 个种子，标签均值仍有采样误差。用组内 std/√n 和正态近似 √(2/π)·std/√n 得到的参考量为 **2.48 HP**；小样本标准差和分布假设会影响这个估计，不能据此认定真实期望的误差，或从主模型 pair MAE 4.54 中直接分离标签噪声。“按标签均值分桶”还可能引入选择效应，因此校准主要查看“按预测值分桶”，两者结合解释。
