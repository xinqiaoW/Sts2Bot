...
|---|---|---|---|---|---|---|
| 绝对误差 | 0.80 | 2.15 | 5.43 | 10.91 | 15.44 | 27.05 |

|误差| ≤ 2 HP 的 pair 占 48.0%，≤ 5 HP 占 72.9%，> 15 HP 占 5.4%。

该快照多数构筑–目标使用 4 个种子，标签均值仍有采样误差。用组内 std/√n 和正态近似 √(2/π)·std/√n 得到的参考量为 **2.30 HP**；小样本标准差和分布假设会影响这个估计，不能据此认定真实期望的误差，或从主模型 pair MAE 4.26 中直接分离标签噪声。“按标签均值分桶”还可能引入选择效应，因此校准主要查看“按预测值分桶”，两者结合解释。
